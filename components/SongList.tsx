                                ...
                                <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-white/10 rounded-xl shadow-2xl overflow-hidden py-1 z-50 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
                                    ...
                                    {FILTERS.map(filter => (
                                     ...
                                     ${activeFilters.has(filter.id)
                                                    ? 'bg-muzzie-copper border-muzzie-copper'
                    ...
                    {listItems.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-64 text-zinc-500 space-y-4 border border-dashed border-zinc-200 dark:border-white/5 rounded-2xl bg-zinc-50 dark:bg-white/[0.02]">
                            ...
                            <button
                                ...
                                className="text-muzzie-copper text-sm font-bold hover:underline"
                            >
                                {t('clearFilters')}
                            </button>
                        </div>
    ...
    return (
        ...
        >
                    ...
                    <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center gap-1">
                        {song.queuePosition ? (
                            ...
                            <div className="flex items-end gap-1 h-6">
                                <div className="w-1 bg-muzzie-copper rounded-full music-bar-anim" style={{ animationDelay: '0.0s' }}></div>
                                <div className="w-1 bg-muzzie-copper rounded-full music-bar-anim" style={{ animationDelay: '0.2s' }}></div>
                                <div className="w-1 bg-muzzie-copper rounded-full music-bar-anim" style={{ animationDelay: '0.4s' }}></div>
                                <div className="w-1 bg-muzzie-copper rounded-full music-bar-anim" style={{ animationDelay: '0.1s' }}></div>
                            </div>
                        )}
                    </div>
                ) : (
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        {isEditingTitle && isOwner ? (
                            <input
                                ...
                                className="font-bold text-lg bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded border border-muzzie-copper focus:outline-none text-zinc-900 dark:text-white min-w-0 flex-1"
                            />
                        ) : (
                            <h3
                                className={`font-bold text-lg truncate ${isCurrent ? 'text-muzzie-copper' : 'text-zinc-900 dark:text-white'} ${isOwner && !song.isGenerating ? 'cursor-pointer hover:underline' : ''}`}
                                onClick={(e) => {
                        ...
                        <span className="inline-flex items-center justify-center text-[9px] font-bold text-white bg-gradient-to-r from-muzzie-copper to-muzzie-green px-1.5 py-0.5 rounded-sm shadow-sm" title={`DiT model: ${song.ditModel || 'undefined'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            <div className="w-4 h-4 rounded-full bg-muzzie-green text-[8px] flex items-center justify-center font-bold text-white">
                    ...
                    {song.isGenerating && (
                        <div className="pt-2">
                            <div className="h-1 rounded-full bg-zinc-200/70 dark:bg-white/10 overflow-hidden">
                                <div
                                    className={`h-full bg-gradient-to-r from-muzzie-copper to-muzzie-green transition-all ${song.progress === undefined ? 'opacity-40' : ''}`}
                                    style={{
                                        width: `${Math.min(
                                            100,
                ...
                {!song.isGenerating && (
                    <div className="flex items-center gap-1 pt-2">
                        <button
                            className={`flex items-center gap-1 px-3 py-1.5 rounded-full hover:bg-white/5 transition-colors ${isLiked ? 'text-muzzie-copper bg-muzzie-copper/10' : 'text-zinc-400 hover:text-black dark:hover:text-white'}`}
            ...
            <div className="text-xs font-mono text-zinc-500 dark:text-zinc-600 self-start pt-1">
                {song.isGenerating ? (
                    <span className={song.queuePosition ? 'text-amber-500' : 'text-muzzie-copper'}>
