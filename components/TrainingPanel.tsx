      ...
      <div className="flex px-4 gap-1 flex-shrink-0">
        {tabs.map(tab => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${activeTab === tab.id ? 'bg-muzzie-copper/20 text-muzzie-gold border border-muzzie-copper/30' : 'text-zinc-400 hover:text-zinc-200 hover:bg-white/5'}`}
          >
            {tab.icon}
            {tab.label}
          </button>
            ...
            <div className="space-y-2 mt-2">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1">
                <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                  <input type="checkbox" checked={initLlm} onChange={e => setInitLlm(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                  Init LLM
                </label>
                <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                  <input type="checkbox" checked={useFlashAttention} onChange={e => setUseFlashAttention(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                  Flash Attention
                </label>
                <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                  <input type="checkbox" checked={offloadToCpu} onChange={e => setOffloadToCpu(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                  Offload CPU
                </label>
                <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                  <input type="checkbox" checked={offloadDitToCpu} onChange={e => setOffloadDitToCpu(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                  Offload DiT CPU
                </label>
                <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                  <input type="checkbox" checked={compileModel} onChange={e => setCompileModel(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                  Compile
                </label>
                <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                  <input type="checkbox" checked={quantization} onChange={e => setQuantization(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                  Quantization
                </label>
              </div>
              {initLlm && (
          ...
          <>
            ...
            <Section title={t('uploadAudio')}>
              <div
                ...
                className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all ${isDragOver ? 'border-muzzie-copper bg-muzzie-copper/10' : 'border-white/10 hover:border-white/20 hover:bg-white/[0.02]'}`}
              >
                <Upload size={24} className={`mx-auto mb-2 ${isDragOver ? 'text-muzzie-gold' : 'text-zinc-500'}`} />
              ...
              {queuedFiles.length > 0 && (
                <div className="mt-2 space-y-2">
                  <FieldRow label={t('datasetName')}>
                    <input type="text" value={uploadDatasetName} onChange={e => setUploadDatasetName(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" placeholder="my_lora_dataset" />
                  </FieldRow>
                  <button onClick={handleUploadAndBuild} disabled={uploading || !uploadDatasetName.trim()} className="w-full py-2 bg-gradient-to-r from-muzzie-copper to-muzzie-green hover:from-muzzie-terracotta hover:to-muzzie-green/80 text-white rounded-lg text-xs font-medium flex items-center justify-center gap-2 disabled:opacity-50">
            ...
            <Section title="Scan Directory">
              <div className="flex gap-2">
                <input type="text" value={scanDir} onChange={e => setScanDir(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" placeholder="./path/to/audio/folder" />
            ...
            <Section title={t('loadExistingDataset')}>
              <div className="flex gap-2">
                <input type="text" value={datasetPath} onChange={e => setDatasetPath(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" placeholder="./datasets/my_dataset.json" />
                <button onClick={handleLoadDataset} disabled={datasetLoading} className="px-3 py-1.5 bg-muzzie-copper/20 hover:bg-muzzie-copper/30 text-muzzie-gold rounded-lg text-xs font-medium flex items-center gap-1.5 disabled:opacity-50">
                  ...
                  <table className="w-full text-[10px]">
                    ...
                    <tbody>
                      {dataframeRows.map((row, i) => (
                        <tr key={i} onClick={() => handleSampleNavigate(i)} className={`cursor-pointer transition-colors ${i === currentSampleIdx ? 'bg-muzzie-copper/10 text-muzzie-gold' : 'hover:bg-white/5 text-zinc-300'}`}>
              ...
              <>
                <Section title={t('datasetSettings')}>
                  <div className="space-y-2">
                    <FieldRow label={t('datasetName')}>
                      <input type="text" value={datasetSettings.datasetName} onChange={e => setDatasetSettings(s => ({ ...s, datasetName: e.target.value }))} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                    </FieldRow>
                    <FieldRow label={t('customActivationTag')}>
                      <input type="text" value={datasetSettings.customTag} onChange={e => setDatasetSettings(s => ({ ...s, customTag: e.target.value }))} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" placeholder="e.g. my_style" />
                    </FieldRow>
                    <FieldRow label={t('tagPosition')}>
                      <select value={datasetSettings.tagPosition} onChange={e => setDatasetSettings(s => ({ ...s, tagPosition: e.target.value as DatasetSettings['tagPosition'] }))} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50">
                    ...
                    <FieldRow label={t('allInstrumental')}>
                      <input type="checkbox" checked={datasetSettings.allInstrumental} onChange={e => setDatasetSettings(s => ({ ...s, allInstrumental: e.target.checked }))} className="w-4 h-4 accent-muzzie-copper" />
                    </FieldRow>
                    <FieldRow label={`${t('genreRatio')} (${datasetSettings.genreRatio}%)`}>
                      <input type="range" min={0} max={100} value={datasetSettings.genreRatio} onChange={e => setDatasetSettings(s => ({ ...s, genreRatio: parseInt(e.target.value) }))} className="flex-1 accent-muzzie-copper" />
                    </FieldRow>
                ...
                <Section title={t('autoLabelWithAI')}>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 mb-2">
                    <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                      <input type="checkbox" checked={skipMetas} onChange={e => setSkipMetas(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                      {t('skipMetas')}
                    </label>
                    <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                      <input type="checkbox" checked={formatLyrics} onChange={e => setFormatLyrics(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                      {t('formatLyrics')}
                    </label>
                    <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                      <input type="checkbox" checked={transcribeLyrics} onChange={e => setTranscribeLyrics(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                      Transcribe Lyrics
                    </label>
                    <label className="flex items-center gap-1.5 text-[10px] text-zinc-400">
                      <input type="checkbox" checked={onlyUnlabeled} onChange={e => setOnlyUnlabeled(e.target.checked)} className="w-3 h-3 accent-muzzie-copper" />
                      {t('onlyUnlabeled')}
                    </label>
                  </div>
                ...
                <Section title={`${t('editSample')} (${currentSampleIdx + 1}/${sampleCount})`}>
                  ...
                  {audioPreviewUrl && (
                    <div className="mb-2 flex items-center gap-2 bg-white/5 rounded-lg px-2 py-1.5">
                      <Volume2 size={14} className="text-muzzie-gold flex-shrink-0" />
                  ...
                  <div className="space-y-2">
                    <FieldRow label={t('caption')}>
                      <input type="text" value={editCaption} onChange={e => setEditCaption(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" placeholder={t('musicDescription')} />
                    </FieldRow>
                    <FieldRow label={t('genre')}>
                      <input type="text" value={editGenre} onChange={e => setEditGenre(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                    </FieldRow>
                    <FieldRow label={t('promptOverride')}>
                      <select value={editPromptOverride} onChange={e => setEditPromptOverride(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50">
                    ...
                    <div>
                      ...
                      <textarea value={editLyrics} onChange={e => setEditLyrics(e.target.value)} rows={3} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50 resize-none" />
                    </div>
                    {editRawLyrics && (
                      <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input type="number" value={editBpm} onChange={e => setEditBpm(parseInt(e.target.value) || 0)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                      </div>
                      <div>
                        <label className="text-[11px] text-zinc-500 mb-0.5 block">Key</label>
                        <input type="text" value={editKey} onChange={e => setEditKey(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" placeholder="e.g. C major" />
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <label className="text-[11px] text-zinc-500 mb-0.5 block">Time Sig</label>
                        <select value={editTimeSig} onChange={e => setEditTimeSig(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50">
                      ...
                      <div>
                        ...
                        <select value={editLanguage} onChange={e => setEditLanguage(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50">
                          {LANGUAGES.map(l => <option key={l.value} value={l.value}>{l.label}</option>)}
                        </select>
                      </div>
                    </div>
                    <FieldRow label={t('allInstrumental')}>
                      <input type="checkbox" checked={editInstrumental} onChange={e => setEditInstrumental(e.target.checked)} className="w-4 h-4 accent-muzzie-copper" />
                    </FieldRow>
                    <button onClick={handleSaveSample} disabled={saving} className="w-full py-1.5 bg-muzzie-copper/20 hover:bg-muzzie-copper/30 text-muzzie-gold rounded-lg text-xs font-medium flex items-center justify-center gap-1.5 disabled:opacity-50">
                ...
                <Section title={t('saveDataset')}>
                  <FieldRow label="Save Path">
                    <input type="text" value={savePath} onChange={e => setSavePath(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                  </FieldRow>
                ...
                <Section title="Preprocess to Tensors">
                  ...
                  <div className="mb-3 p-2 bg-white/[0.02] border border-white/5 rounded-lg space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input type="text" value={preprocessDatasetPath} onChange={e => setPreprocessDatasetPath(e.target.value)} placeholder="./datasets/my_lora_dataset.json" className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                  ...
                  <FieldRow label="Output Dir">
                    <input type="text" value={preprocessOutputDir} onChange={e => setPreprocessOutputDir(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                  </FieldRow>
        ...
        {activeTab === 'train' && (
          <>
            ...
            <Section title={t('preprocessedDataset')}>
              <div className="flex gap-2">
                <input type="text" value={trainingParams.tensorDir} onChange={e => setTrainingParams(p => ({ ...p, tensorDir: e.target.value }))} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
            ...
            <Section title={t('trainingParameters')}>
              <div className="space-y-2">
                <FieldRow label={t('learningRate')}>
                  <input type="number" value={trainingParams.learningRate} onChange={e => setTrainingParams(p => ({ ...p, learningRate: parseFloat(e.target.value) || 0.0003 }))} step={0.0001} className="w-28 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                </FieldRow>
                ...
                <FieldRow label="Seed">
                  <input type="number" value={trainingParams.seed} onChange={e => setTrainingParams(p => ({ ...p, seed: parseInt(e.target.value) || 42 }))} className="w-24 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                </FieldRow>
                <FieldRow label={t('outputDirectory')}>
                  <input type="text" value={trainingParams.outputDir} onChange={e => setTrainingParams(p => ({ ...p, outputDir: e.target.value }))} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                </FieldRow>
                <FieldRow label="Resume Checkpoint">
                  <input type="text" value={trainingParams.resumeCheckpoint} onChange={e => setTrainingParams(p => ({ ...p, resumeCheckpoint: e.target.value }))} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" placeholder="./lora_output/checkpoints/epoch_200" />
                </FieldRow>
              </div>
            </Section>
            ...
            <div className="flex gap-2">
              {!isTraining ? (
                <button onClick={handleStartTraining} className="flex-1 py-2.5 bg-gradient-to-r from-muzzie-copper to-muzzie-green hover:from-muzzie-terracotta hover:to-muzzie-green/80 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-2">
                  <Play size={16} />
                  {t('startTraining')}
                </button>
              ) : (
            ...
            <Section title="Export LoRA">
              <div className="space-y-2">
                <FieldRow label="Export Path">
                  <input type="text" value={exportPath} onChange={e => setExportPath(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                </FieldRow>
                <FieldRow label="LoRA Output Dir">
                  <input type="text" value={exportOutputDir} onChange={e => setExportOutputDir(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-muzzie-copper/50" />
                </FieldRow>
              </div>
...
const ParamSlider: React.FC<{
  ...
  <div>
    ...
    <input type="range" min={min} max={max} step={step} value={value} onChange={e => onChange(parseFloat(e.target.value))} className="w-full accent-muzzie-copper h-1.5" />
  </div>
);
