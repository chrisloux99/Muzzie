        ...
        <div className="mb-8">
          <div className="relative max-w-3xl">
            ...
            <input
              ...
              className="w-full h-11 pl-12 pr-12 bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-white/10 rounded-full text-zinc-900 dark:text-white placeholder-zinc-400 focus:outline-none focus:border-muzzie-copper focus:ring-2 focus:ring-muzzie-copper/20 transition-all"
            />
            {searching ? (
              <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 text-muzzie-copper animate-spin" size={18} />
            ) : searchQuery && (
              <button
                ...
                <button
                  ...
                  className={`px-3 py-1.5 border rounded-full text-sm transition-all duration-200 group flex items-center gap-1.5 ${
                    searchQuery === genreLabel
                      ? 'bg-muzzie-copper border-muzzie-copper text-white'
                      : 'bg-zinc-100 dark:bg-zinc-800/60 border-zinc-200 dark:border-white/5 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700/60 hover:border-muzzie-copper/30 hover:text-muzzie-copper dark:hover:text-muzzie-gold'
                  }`}
                >
                  {genreLabel}
                  <Copy
...
const FeaturedSongCard: React.FC<FeaturedSongCardProps> = ({
  ...
  return (
    <div
      className="flex items-center gap-3 p-2 bg-white dark:bg-zinc-900/40 rounded-xl border border-zinc-100 dark:border-white/5 hover:border-muzzie-copper/30 hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-all cursor-pointer group"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          ...
          {song.isPublic !== false && (
            <span className="flex-shrink-0 text-[8px] font-bold text-white bg-gradient-to-r from-muzzie-copper to-muzzie-green px-1 py-0.5 rounded">
              v5
            </span>
          )}
        </div>
        <div className="text-[11px] text-zinc-500 dark:text-zinc-400 truncate mb-1">
          {tags.map((tag, i) => (
            <button
              ...
              className="hover:text-muzzie-copper dark:hover:text-muzzie-gold transition-colors"
            >
              {tag}{i < tags.length - 1 ? ', ' : ''}
        ...
        <div className="flex items-center gap-2 text-[10px] text-zinc-400">
          {song.creator && (
            <button
              ...
              className="flex items-center gap-1 hover:text-muzzie-copper transition-colors max-w-[80px]"
            >
              {song.creator_avatar ? (
                <img
                ...
                <div className="w-3.5 h-3.5 rounded-full bg-gradient-to-br from-muzzie-copper to-muzzie-green flex items-center justify-center text-[7px] text-white font-bold flex-shrink-0">
...
const CreatorCard: React.FC<CreatorCardProps> = ({
  ...
  return (
    ...
    >
      <div className="w-[90px] h-[90px] mx-auto rounded-full overflow-hidden mb-2 ring-2 ring-transparent group-hover:ring-muzzie-copper transition-all shadow-lg">
        <img
      ...
      <div className="font-semibold text-zinc-900 dark:text-white text-sm truncate group-hover:text-muzzie-copper transition-colors px-1">
        {creator.username}
      </div>
...
const PlaylistCard: React.FC<PlaylistCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="font-semibold text-zinc-900 dark:text-white text-sm truncate group-hover:text-muzzie-copper transition-colors">
        {playlist.name}
      </div>
      ...
      {playlist.creator && (
        ...
        >
          ...
          <span className="text-[11px] text-zinc-500 hover:text-muzzie-copper transition-colors truncate">
            {playlist.creator}
          </span>
        </div>
      )}
