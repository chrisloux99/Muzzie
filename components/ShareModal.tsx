...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, song }) => {
  ...
  const defaultShareText = `🎵 "${song.title}" ${song.style ? `(${song.style})` : ''} - Made with Muzzie`;
  const twitterText = `🔥 Just created "${song.title}" with Muzzie - AI music generation! ${song.style ? `#${song.style.replace(/\s+/g, '')}` : ''} #AIMusic #Muzzie`;
  const redditTitle = `[AI Music] ${song.title} - ${song.style || 'Original'} | Created with Muzzie`;
  const whatsAppText = `🎧 Listen to this AI-generated song!\n\n"${song.title}" by ${song.creator || 'Unknown Artist'}\n${song.style ? `Genre: ${song.style}` : ''}\n\nMade with Muzzie - free and open source!`;
  const telegramText = `🎵 "${song.title}" by ${song.creator || 'Unknown Artist'}\n${song.style ? `🎸 ${song.style}` : ''}\n\n🤖 Made with Muzzie`;
  const linkedInText = `Check out this AI-generated music: "${song.title}" - Created locally with Muzzie. #AIMusic #MusicTech #OpenSource`;
